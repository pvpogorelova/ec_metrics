...
data_ma = tibble(x = 50 + arima.sim(n = n_obs, model = list(ma = 0.9), sd = 2))
...
# x: ARMA(1,1): x_t = 1 + 0.7*x_(t-1) + u_t - 0.2*u_(t-1), u_t ~ WN(0, 4)
# y: ARMA(2,1): y_t = 0.4*y_(t-1) - 0.03*y_(t-2) + u_t - 0.7u_(t-1), u_t ~ WN(0, 1)

data_arma = tibble(x = 1 + arima.sim(n = n_obs,
                                   model = list(ar = 0.7, ma = -0.2, sd = 2)),
                 y = 0 + arima.sim(n = n_obs,
                               model = list(ar = c(0.4, -0.03), ma = -0.7, sd = 1)))
